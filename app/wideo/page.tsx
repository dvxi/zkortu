...
const tournaments = ["Wszystkie turnieje", "Roland Garros 2026", "Australian Open 2026", "Bad Homburg Open 2026", "Queens Club 2026"];
...
export default function WideoPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="flex flex-wrap gap-2">
          {tournaments.map((t) => (
            <button
              ...
              style={tournament === t ? { backgroundColor: "var(--gold)", borderColor: "var(--gold)", color: "var(--brand)" } : {}}
            >
              {t}
            </button>
          ))}
      ...
      {filtered.length === 0 ? (
        <div key={`empty-${category}-${tournament}`} className="anim-switch text-center py-20 text-muted-foreground">
        ...
        <div key={`${category}-${tournament}`} className="anim-switch">
          ...
          {featured && (
            <a
              href={`https://www.youtube.com/results?search_query=${encodeURIComponent(featured.youtubeSearch)}`}
              target="_blank"
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-0 rounded-xl overflow-hidden border border-border">
                <div className="relative lg:col-span-3 h-64 lg:h-80 overflow-hidden">
                  <Image
                    src={`https://picsum.photos/seed/${picsumSeeds[featured.id] ?? "tennis-vid-iga"}/900/480`}
                    alt={featured.title}
                    fill
                    className="object-cover group-hover:scale-105 transition-transform duration-500"
                  />
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-16 h-16 rounded-full flex items-center justify-center transition-transform group-hover:scale-110" style={{ backgroundColor: "rgba(15,42,27,0.80)" }}>
                  ...
                  <div className="absolute top-3 left-3">
                    <span className={`text-xs font-bold px-2 py-1 rounded ${categoryColors[featured.category]}`}>
                      {featured.category}
                    </span>
                  </div>
                </div>
              ...
              {rest.map((video) => (
                ...
                >
                  <div className="relative h-44 overflow-hidden">
                    <Image
                      src={`https://picsum.photos/seed/${picsumSeeds[video.id] ?? "tennis-vid-wta"}/600/400`}
                      alt={video.title}
                      fill
                      className="object-cover group-hover:scale-105 transition-transform duration-500"
                    />
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-12 h-12 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" style={{ backgroundColor: "rgba(15,42,27,0.85)" }}>
                    ...
                    <div className="absolute top-2 left-2">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded ${categoryColors[video.category]}`}>
                        {video.category}
                      </span>
                    </div>
                  </div>
